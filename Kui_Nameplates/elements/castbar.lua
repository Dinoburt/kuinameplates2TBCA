...
function addon.Nameplate.CastBarShow(f)
    ...
    if f.elements.CastBar then
        ...
        if f.cast_state.channel then
            f.CastBar:SetValue(
                (f.cast_state.end_time-f.cast_state.start_time)
                -(GetTime()-f.cast_state.start_time)
            )
...
function ele:Show(f)
    ...
    if UnitChannelInfo(f.unit) then
        ...
        if numStages and numStages > 0 then
            if kui.CLASSIC and not IS_TBC then return end
...
function ele:CastStart(event,f,unit)
    local name,text,texture,startTime,endTime,guid,notInterruptible,numStages

    if event == 'UNIT_SPELLCAST_CHANNEL_START'
    or event == 'UNIT_SPELLCAST_EMPOWER_START' then
        name,text,texture,startTime,endTime,_,notInterruptible,_,numStages =
            UnitChannelInfo(unit)
    else
        name,text,texture,startTime,endTime,_,guid,notInterruptible =
...
function ele:CastStop(event,f,_,guid)
    ...
    f.handler:CastBarHide(
        ...
        or event == 'UNIT_SPELLCAST_FAILED') and ele.HIDE_INTERRUPT)
        or (event == 'UNIT_SPELLCAST_SUCCEEDED' and ele.HIDE_SUCCESS)
        or ele.HIDE_STOP
    )
end
...
local function LibCC_CastStart(...)   LibCC_Resolve('CastStart',...) end
local function LibCC_CastStop(...)    LibCC_Resolve('CastStop',...) end
local function LibCC_CastUpdate(...)  LibCC_Resolve('CastUpdate',...) end
local function LibCC_ChannelStop(...) LibCC_Resolve('UNIT_SPELLCAST_CHANNEL_STOP',...) end

-- init
function ele:Initialise()
    if kui.CLASSIC and not kui.CATA and not IS_TBC then
        LibCC = LibStub('LibClassicCasterino',true)
        if not LibCC then return end

        UnitCastingInfo = function(unit) return LibCC:UnitCastingInfo(unit) end
        UnitChannelInfo = function(unit) return LibCC:UnitChannelInfo(unit) end
    else
...
function ele:OnEnable()
    if not kui.CLASSIC or kui.CATA or IS_TBC then
